...
struct AbstractTlsSocket::Private {
  ...
  TlsContext ctx_;
  SSL *ssl_ = nullptr;
  BIO *internal_bio_ = nullptr; // Сюда мы будем складывать сырые данные из сокета
  BIO *network_bio_ = nullptr;  // Отсюда OpenSSL будет забирать зашифрованный ответ для отправки в сеть
...
void AbstractTlsSocket::activateEvent() {
  ...
  if (!private_.ssl_) {
    ...
    BIO_new_bio_pair(&private_.internal_bio_, 0, &private_.network_bio_, 0);
    SSL_set_bio(private_.ssl_, private_.network_bio_, private_.network_bio_);
  ...
  if (sys_available > 0) {
    ...
    if (r_sys > 0) {
      BIO_write(private_.internal_bio_, handshake_buf.data(), r_sys);
    }
  }
  // Также проталкиваем остатки из raw_input_buffer_, если они там были
  if (!private_.raw_input_buffer_.empty()) {
    int written = BIO_write(private_.internal_bio_, private_.raw_input_buffer_.data(), private_.raw_input_buffer_.size());
    if (written > 0) {
      private_.raw_input_buffer_.erase(private_.raw_input_buffer_.begin(), private_.raw_input_buffer_.begin() + written);
    }
  }
  ...
  int pending = BIO_ctrl_pending(private_.internal_bio_);
  if (pending > 0) {
    std::vector<uint8_t> enc_buf(pending);
    int read_bytes = BIO_read(private_.internal_bio_, enc_buf.data(), pending);
    if (read_bytes > 0) {
...
int AbstractTlsSocket::read_available_fd() const {
  if (!private_.encrypt_) {
    return AbstractSocket::read_available_fd();
  }

  auto* pr = const_cast<AbstractTlsSocket::Private*>(&private_);
  ...
  if (sys_available < 0) {
    ...
    size_t old_size = pr->raw_input_buffer_.size();
    pr->raw_input_buffer_.resize(old_size + sys_available);

    int r = const_cast<AbstractTlsSocket*>(this)->AbstractSocket::read_fd(
        pr->raw_input_buffer_.data() + old_size, sys_available);

    if (r < 0) {
      sys_disconnected = true;
      pr->raw_input_buffer_.resize(old_size); // Откатываем размер назад
    } else if (r == 0) {
      // Внезапный EOF (FIN пакет от удаленной стороны)
      sys_disconnected = true;
      pr->raw_input_buffer_.resize(old_size);
    } else {
      pr->raw_input_buffer_.resize(old_size + r);
    }
  }

  // 2. Проталкиваем всё, что можем, в OpenSSL BIO
  if (!pr->raw_input_buffer_.empty()) {
    int written = BIO_write(pr->internal_bio_, pr->raw_input_buffer_.data(), pr->raw_input_buffer_.size());
    if (written > 0) {
      pr->raw_input_buffer_.erase(pr->raw_input_buffer_.begin(), pr->raw_input_buffer_.begin() + written);
    }
  }
  ...
  if (pr->raw_input_buffer_.empty() && BIO_ctrl_pending(pr->internal_bio_) == 0) {
    if (sys_disconnected) {
...
int AbstractTlsSocket::write_fd(const void *data, int size) {
  if (!private_.encrypt_) {
    return AbstractSocket::write_fd(data, size);
  }
