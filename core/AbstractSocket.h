...
public:
  ...
  /** @brief Writes raw binary bytes out to the network socket layer. @param @p data Source raw memory buffer pointer containing data to transmit. @param size The number of bytes to transmit from the data buffer. @return Number of bytes successfully dispatched to the socket queue, or a negative value on error. */
  int write(const uint8_t *, int);
  /** @brief Transmits a structural DataArray package out to the network layer. @param @p data Reference to the DataArray containing the payload to write. @return Number of bytes successfully dispatched to the socket queue, or a negative value on error. */
...
protected:
  ...
  /** @brief Low-level native operating system read operation proxying requests directly down to the fd handle. @param @p data Destination memory allocation chunk pointer. @param size Bounds metric tracking maximum allowed byte read capacity. @return Number of raw bytes read from the descriptor, or error status codes. */
  virtual int read_fd(void *, int) const;
  /** @brief Low-level native operating system write operation proxying requests directly up to the fd handle. @param @p data Source memory block holding raw serialization binary data. @param size Size in bytes to write identifying exactly how many bytes to transfer. @return Number of raw bytes written out to the descriptor, or error status codes. */
