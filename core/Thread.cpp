...
AbstractThread::~AbstractThread() {
  console_msg_(std::string(__PRETTY_FUNCTION__)  + "  " + LOG_THREAD_NAME + " 1");
  ...
  console_msg_(std::string(__PRETTY_FUNCTION__)  + "  " + LOG_THREAD_NAME + " 3");
  {  //lock scope
    LockGuard lock(Private::list.mutex);
    std::vector<AbstractThread *>::iterator it = std::lower_bound(Private::list.begin(), Private::list.end(), this, Private::Compare());
    ...
    trace() << "threads:" << std::to_string(Private::list.size());
    console_msg_(std::string(__PRETTY_FUNCTION__)  + "  " + LOG_THREAD_NAME + " 4");
  }
  ...
  console_msg_(std::string(__PRETTY_FUNCTION__)  + "  " + LOG_THREAD_NAME + " 5");
  ...
  console_msg_(std::string(__PRETTY_FUNCTION__)  + "  " + LOG_THREAD_NAME + " 6");

  delete &private_;
  console_msg_(std::string(__PRETTY_FUNCTION__)  + "  " + LOG_THREAD_NAME + " 7");
}
...
void AbstractThread::requestInterrupt() {
  ...
  if (private_.state != Private::Interrupted) {
    if (!private_.state || private_.state >= Private::WaitFinished) {
      console_msg("AbstractThread", "not running");
      return;
    }
...
void AbstractThread::waitInterrupted() const {
  ...
  if (private_.state != Private::WaitInterrupted) {
    console_msg("AbstractThread", "interrupt request not set");
    return;
  }
...
void AbstractThread::quit() {
  ...
  if (!private_.state || private_.state == Private::Finished) {
    console_msg("AbstractThread", "thread already finished or not started");
    return;
  }
...
void AbstractThread::waitFinished() const {
  ...
  checkDifferentThread();
  if (!private_.state) {
    console_msg("AbstractThread", "thread not running");
    return;
  }
...
void AbstractThread::start() {
  ...
  if ((private_.state && private_.state != Private::Finished) || private_.id != std::thread::id()) {
    console_msg("AbstractThread", "thread already running or starting, or thread id not null");
    return;
  }
...
bool AbstractThread::modifyTimer(int id, int ms) {
  ...
  console_msg("AbstractThread", "timer: " + std::to_string(id) + " not found " + LOG_THREAD_NAME);
  return false;
}
