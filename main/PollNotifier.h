...
/** @file PollNotifier.h @brief The PollNotifier class. */
...
public:
  PollNotifier();
  /** @brief Constructs a poll notifier. It assigns the file descriptor, and watches for events. @param fd The file descriptor to poll @param events watch events */
  PollNotifier(int, AbstractThread::PollEvents = AbstractThread::PollIn);
  ~PollNotifier();
  /** @brief Assigns the file descriptor and watch events. @param fd The file descriptor to poll @param events watch events @return True if descriptor is set */
