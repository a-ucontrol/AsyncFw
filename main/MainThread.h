...
namespace AsyncFw {
/*! \class MainThread MainThread.h <AsyncFw/MainThread>.
\brief A static control interface managing the application's primary execution thread (Main Thread). MainThread initializes the master Event Loop on the application's boot thread (typically `main()`). It handles core process-wide shutdown interceptors and manages the primary event loop execution frame.
