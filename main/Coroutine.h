...
/*! \example Coroutine/main.cpp Coroutine example \example CoroutineWait/main.cpp CoroutineWait example */
...
struct CoroutineAwait {
  template <typename T>
  CoroutineAwait(T f) : f_(new Function<const CoroutineHandle>::Value(std::forward<T>(f))) {}
